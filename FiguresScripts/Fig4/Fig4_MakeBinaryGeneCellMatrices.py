...
# -*- coding: utf-8 -*-
"""
make_binary_gene_cell_matrices_TONSIL_COMMON3.py  (Tonsil: ALL + byLeidenMergedType)
...
Restricts to genes present in ALL THREE modalities (COMMON3), so outputs share the
same gene rows and ordering.
...
# ------------------ CONFIG (TONSIL) ------------------
RNA_PATH = "RNA_Tonsil.h5ad"
AC_PATH  = "H3K27ac_Tonsil.h5ad"
ME3_PATH = "H3K27me3_Tonsil.h5ad"

OUTROOT = "Tonsil_BinaryMatrices_OUT"   # will create ALL/ and byLeidenMergedType/
...
# Gene-activity window (±5 kb around gene body)
GA_FLANK = 5_000
...
def _maybe_strip_suffix_after_last_underscore(adata, ref_index: pd.Index, col_name="barcode_orig"):
    ...
    (Safe for cases where AC/ME3 obs_names have suffixes.)
    """
    adata = adata.copy()
...
print(f"[info] common cells: {common_cells.size:,}")

adata_rna = adata_rna[common_cells].copy()
adata_ac  = adata_ac[common_cells].copy()
adata_me3 = adata_me3[common_cells].copy()

# Ensure group metadata present in all modalities
group_raw = _pull_obs_col(GROUP_COL, (adata_rna, adata_ac, adata_me3)).astype(str)
group_s   = group_raw.map(_sanitize)

for ad in (adata_rna, adata_ac, adata_me3):
    ad.obs[GROUP_COL] = group_raw.values
    ad.obs["group_leiden_merged_type"] = group_s.values

print("[info] cells per leiden_merged_type (sanitized):")
print(pd.Series(group_s.values).value_counts().sort_index())
...
def make_gene_activity_tables(
    ...
    ga = ga[cell_order, :]  # align cells to RNA order
...
print("[run] building gene activity matrices (AC/ME3) ...")
...
if len(COMMON3_GENES_ALL) == 0:
    raise RuntimeError("[ERR] COMMON3 gene set is empty. Gene symbols may be mismatched across modalities.")
...
# Restrict base matrices to COMMON3 once
...
# ------------------ EXPORT: byLeidenMergedType (grouped columns) ------------------
out_blt = os.path.join(OUTROOT, "byLeidenMergedType")
os.makedirs(out_blt, exist_ok=True)

groups = pd.Series(adata_rna.obs["group_leiden_merged_type"].values, index=adata_rna.obs_names)

group_levels = sorted(groups.unique().tolist())
cell_names_blt: List[str] = []
cell_labels_blt: List[str] = []

for g in group_levels:
    cells_g = [c for c in CELL_ORDER_ALL if groups.loc[c] == g]
    cell_names_blt.extend(cells_g)
    cell_labels_blt.extend([f"{g}|{c}" for c in cells_g])
...
pd.DataFrame({"group": group_sizes.index, "n_cells": group_sizes.values}).to_csv(
    os.path.join(out_blt, "group_sizes.tsv"), sep="\t", index=False
)
pd.DataFrame({
    "barcode": CELL_ORDER_ALL,
    "group_leiden_merged_type": groups.reindex(CELL_ORDER_ALL).values,
    "leiden_merged_type_raw": adata_rna.obs[GROUP_COL].reindex(CELL_ORDER_ALL).astype(str).values,
}).to_csv(os.path.join(out_blt, "cell_to_group.tsv"), sep="\t", index=False)

export_folder(
    outdir=out_blt,
    cell_names=cell_names_blt,
    cell_labels=cell_labels_blt,
    gene_order=GENE_ORDER_ALL,
    markers_common3=MARKERS_COMMON3_ALL,
    rna_strats=RNA_STRATS_ALL,
    ac_strats=AC_STRATS_ALL,
