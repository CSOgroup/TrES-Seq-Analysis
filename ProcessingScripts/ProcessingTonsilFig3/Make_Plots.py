...
def make_upsetplot(
    ...
    df = df.with_columns(
        pl.col("SB").str.slice(1)
    )
    df = df.drop(["L1","L2","L3"]) #remove ligaation columns
    #df = df.drop("UM", strict=False)
    CheckinGenes = True

    if "VTHumanTonsil" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["GCAT", "CGAT", "GACT", "AGCT", "CAGT", "ACGT", "GCTA", "CGTA", "GTCA", "TGCA", "CTGA"]))
    elif "VTK422" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["TCGA"]))
    elif "A20" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["AGCAT","TGCAT","CGCAT"]))
    elif "A549" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["ACGAT","TCGAT","CCGAT"]))
    elif "GM12878" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["AGACT","TGACT","CGACT"]))
    elif "HCC827" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["AAGCT","TAGCT","CAGCT"]))
    elif "JJN2" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["ACAGT","TCAGT","CCAGT"]))
    elif "Karpas422" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["AACGT","TACGT","CACGT"]))
    elif "KPAR" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["AGCTA","TGCTA","CGCTA"]))
    elif "WSU" in bam_file_path:
        df = df.filter(pl.col("SB").is_in(["AGTCA","TGTCA","CGTCA"]))
    # else: continue without filtering
    ...
    if 'DNA' in select_modality:
        ...
        df = df.join(create_alignment_df(bam_file_path.replace("H3K27ac", "H3K27me3")),
        left_on='Record_Name',
        right_on='Read_Name_BAM',
        how='left',
        validate='1:1',
        ...
        cntBC = pl.read_csv(good_bc_path.replace("H3K27ac", "H3K27me3"), separator='\t', has_header=False, new_columns=["MappedNoFilt", "bc"])
        cntBC = cntBC.with_columns(pl.lit(True).alias("MappedNoFilt"))
...
def make_barplots(subdir_path,plot_dir):
    ...
    for file_path in tsv_files:
        ...
        exp1_map = {
            sb_bcs[0]: 'HumanTonsil',
            sb_bcs[1]: 'HumanTonsil',
            sb_bcs[2]: 'HumanTonsil',
            sb_bcs[3]: 'HumanTonsil',
            sb_bcs[4]: 'HumanTonsil',
            sb_bcs[5]: 'HumanTonsil',
            sb_bcs[6]: 'HumanTonsil',
            sb_bcs[7]: 'HumanTonsil',
            sb_bcs[8]: 'HumanTonsil',
            sb_bcs[9]: 'HumanTonsil',
            sb_bcs[10]: 'HumanTonsil',
            sb_bcs[11]: 'K422'
        }
...
if __name__ == "__main__":
    ...
    # DNA samples
    sample_list_DNA1 = ["Sc_VTD9_S1_Sc_VTD11_S2_VTHumanTonsil","Sc_VTD9_S1_Sc_VTD11_S2_VTK422"]

    # RNA samples
    sample_list_RNA1 = ["Sc_VTr10_S2_Sc_VTr12_S3_VTHumanTonsil","Sc_VTr10_S2_Sc_VTr12_S3_VTK422"]

    mod_list = ["H3K27ac","H3K27me3"]
    out_folder='/mnt/dataFast/ahrmad/triseq_2025052_2/processed'

    # Barplots
    make_barplots(out_folder,out_folder)

    for sample in sample_list_DNA1:
        Tag_records_path = f'{out_folder}/Tag_Records_Sc_VTD9_S1_Sc_VTD11_S2.tsv.gz'
    ...
    for sample in sample_list_RNA1:
        Tag_records_path = f'{out_folder}/Tag_Records_Sc_VTr10_S2_Sc_VTr12_S3.tsv.gz'
