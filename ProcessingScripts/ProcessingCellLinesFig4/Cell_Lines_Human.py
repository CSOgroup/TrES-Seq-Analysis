...
# List of human cell line sample names (prefixes)
cell_lines = [
    "Sc_CMD_GM12878",
    "Sc_CMD_JJN2",
    "Sc_CMD_Karpas422",
...
cell_line_samples = [
    ("Sc_CMr_GM12878", "Sc_CMr_GM12878_NoDupUMI_count.tsv"),
    ("Sc_CMr_JJN2", "Sc_CMr_JJN2_NoDupUMI_count.tsv"),
    ("Sc_CMr_Karpas422", "Sc_CMr_Karpas422_NoDupUMI_count.tsv"),
    ("Sc_CMr_WSU", "Sc_CMr_WSU_NoDupUMI_count.tsv"),
]
...
for sample in cell_lines:
    ...
    if sample in ["WSU"]:
        minFrags_ac = 100
    elif sample in ["Karpas422"]:
        minFrags_me3 = 500
    elif sample in ["GM12878"]:
        minFrags_me3 = 100
...
sc.pl.umap(
    ...
    ncols=3,
    show=False, save=f'AllCellLines_UMAP_QC_Log.pdf'
)
sc.pl.umap(
    adata,
...
sc.pl.umap(
    ...
    legend_loc=  "on data",
    ncols=2,
    show=False, save=f'AllCellLines_UMAP_QC_RNA_Leiden.pdf'
)

# Save the resulting AnnData object
adata.write("AllCellLines_rna_merged_processed.h5ad")

#################################################
############# VIOLIN RNA HUMAN CELL LINES ###############
#################################################
...
#### Histone Mark Analysis for All Human Cell Lines (Joint UMAP per mark)
...
for mark in ['ac','me3']:
    ...
    for sample in cell_lines:
        ...
        tt = f"H3K27{mark}"
        snap.pp.make_peak_matrix(adata_sample, peak_file=f'/mnt/dataFast/ahrmad/triseq_2025052/processed/cell_lines/peaks/combined_{tt}_ITMPeaks.bed', counting_strategy='paired-insertion', chunk_size=500000, inplace=True)
        adatas.append(adata_sample)
    ...
    sc.pl.umap(
        ...
        ncols=2,
        save=f'AllCellLines_{mark_name}_UMAP_QC_peaks.pdf',
        show=False
    )
    ...
    sc.pl.umap(
        ...
        legend_loc="on data",
        ncols=2,
        save=f'AllCellLines_{mark_name}_UMAP_Leiden_peaks.pdf',
        show=False
    )
    # Save the merged AnnData object for this mark
    adata.write(f"AllCellLines_{mark_name}_merged_processed_PEAKS.h5ad")
    jaccard_types(adata,mark_name+'_peaks',Experiment='Cell_Lines_H2R_202505')


for mark_name in ["H3K27ac","H3K27me3"]:
    adata = sc.read(f"AllCellLines_{mark_name}_merged_processed_PEAKS.h5ad")
    snap.metrics.frip(adata, {"FRiP": f'/mnt/dataFast/ahrmad/triseq_2025052/processed/cell_lines/peaks/combined_{mark_name}_ITMPeaks.bed'}, inplace=True)
    sc.pl.umap(
        adata,
        color=["FRiP", "sample"],
        ncols=2,
        save=f'AllCellLines_{mark_name}_UMAP_Leiden_peaks_FRIP.pdf',
        show=False
    )


#################################################
############# VIOLIN DNA HUMAN CELL LINES ###############
#################################################

for pp in ["H3K27ac","H3K27me3"]:
    #adata = sc.read_h5ad(f"AllCellLines_Mouse_{pp}_merged_processed.h5ad")
    
    adata1 = sc.read_h5ad(f'AllCellLines_{pp}_merged_processed.h5ad')
    adata2 = sc.read_h5ad(f'AllCellLines_Mouse_{pp}_merged_processed.h5ad')
    ...
    plt.tight_layout()
    plt.savefig(f'./figures/AllCellLines_ALL_VIOLIN_{pp}.pdf')
    plt.close()
...
adata1 = sc.read_h5ad('AllCellLines_rna_merged_processed.h5ad')
adata2 = sc.read_h5ad('AllCellLines_Mouse_rna_merged_processed.h5ad')
...
############# DNA SHARED ###############
#################################################
adata1 = sc.read_h5ad(f"AllCellLines_H3K27ac_merged_processed.h5ad")
adata2 = sc.read_h5ad(f"AllCellLines_H3K27me3_merged_processed.h5ad")
adata3 = sc.read_h5ad("AllCellLines_rna_merged_processed.h5ad")
...
for adata,pp in [(adata1,'H3K27ac'),(adata2,'H3K27me3'),(adata3,'RNA')]:
    ...
    sc.pl.umap(
        ...
        legend_loc=  "on data",
        ncols=2,
        show=False, save=f'AllCellLines_UMAP_QC_{pp}_Leiden_SHARED.pdf'
    )
...
for adataPP,pp in [(adata1,'H3K27ac'),(adata2,'H3K27me3'),(adata3,'RNA')]:
    ...
    plt.title(f"Library Size per Sample ({pp})")
    plt.tight_layout()
    plt.savefig(f'./figures/AllCellLines_VIOLIN_{pp}_SHARED.pdf')
    plt.close()
...
plt.title(f"Library Size per Sample ({pp})")
plt.tight_layout()
plt.savefig(f'./figures/AllCellLines_VIOLIN_{pp}.pdf')
plt.close()
